...
```
경건생활 앱 주소: https://<GitHub 아이디>.github.io/godly-life-family/
가입 코드: <선생님이 정한 코드>
...
**"아직 설정되지 않은 항목이 있습니다: …"**
→ 1번의 `GAS_URL` 또는 `OAUTH_CLIENT_ID`가 아직 `PASTE_...` 상태입니다.
   화면에 어느 항목인지 함께 표시되니 그 값을 채우고 push하세요.

**로그인 버튼이 안 보여요 («로그인 준비 중...»에서 멈춤)**
→ 구글 로그인 스크립트를 못 받은 것입니다. 인터넷이 느리면 생길 수 있습니다.
   15초 뒤 «다시 시도» 버튼이 나오니 눌러 보세요. 계속 그러면 회사·학교 와이파이가
   `accounts.google.com`을 막고 있을 수 있으니 다른 망(LTE 등)으로 시도해 보세요.

**로그인 버튼은 보이는데 눌러도 안 되거나 오류가 떠요**
→ 5번(승인된 JavaScript 원본)을 안 했거나 주소가 틀렸습니다. 주소 끝에 `/`가 붙으면 안 됩니다.

**고쳤는데 화면이 그대로예요**
→ 앱 코드는 인터넷에 연결되어 있으면 **새로고침 한 번으로 최신이 됩니다.**
   그래도 그대로면 GitHub Pages 반영(1~2분)을 기다렸다가 다시 열어 보세요.
   참고자료(자료 탭 본문)만 기기에 저장되므로, 자료를 바꾼 경우에는
   `web/sw.js`의 `BUILD_TAG`를 올려서 push해야 갱신됩니다.
