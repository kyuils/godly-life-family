...
- 모든 문서에 `source.license`를 **실제로 확인한 내용만** 기재한다. 확인하지 못하면 `"확인 불가"` +
  `licenseConfidence: "제한적"`로 적는다. 임의로 "퍼블릭 도메인"이라 단정하지 않는다.
- 하이델베르크 요리문답: 원문(1563)은 저작권이 소멸했으나 **한글 번역본에는 번역 저작권이 살아 있을 수 있다.**
  이용 조건이 명확히 자유로운 판본을 확보하지 못하면 **v1에서 제외한다.**
- `licenseConfidence`가 `제한적`인 문서가 있으면 사용자에게 보고하고 판단을 받는다
  (선택지: 해당 문서 제외 / 저장소를 비공개로 두고 Pages만 공개 / 그대로 진행).
