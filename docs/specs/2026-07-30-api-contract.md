...
3. `api.call`은 세대가 어긋나면 **요청을 만들지 않고** 즉시 `{ ok:false, code:'stale_session' }`을
   ...
   - **★ 검사는 «보내기 직전»마다 한다. 함수 진입 시 1회로는 부족하다** (v1.5.1에서 정정).
     `busy` 재시도는 1~3초를 기다렸다가 **다시 보내는데**, `post()`는 그때의 전역 토큰을
     읽는다. 진입 시에만 검사하면 재전송이 남의 토큰으로 나간다 — 함수 **안에서**
     «한 곳이 빠진다»가 그대로 재현된 것이다.
     읽기 액션은 `loadMyData`의 `owner` 검사가 뒤에서 한 번 더 거르지만,
     **`register`는 쓰기라 그대로 시트에 남는다** — B 계정이 A가 입력한 이름·학년반·학교로
     명부에 등재되고 화면에는 아무 표시도 나지 않는다.
     (2026-08-02 검토자 에이전트가 재현 스크립트로 잡았다. v1.5.0의 회귀 테스트는
      **재시도를 한 번도 태우지 않아** 통과하면서 이 구멍을 놓쳤다 — 새 테스트는
      `busy`를 일부러 만들어 재시도 경로를 태운다)
...
5. 흐름을 버리는 곳(«돌아가기», 다른 계정 콜백)은 **반드시 `signInSeq`를 올린다.**
   ...
   - **★ 새 흐름은 `afterSignIn` 시작에서 `clearLoading()`으로 앞 흐름의 잔재를 지운다**
    (v1.5.2에서 정정). 버려진 흐름은 `finally`에서 `clearLoading()`을 건너뛰므로
    (`abandoned()`가 참), 「다른 계정 콜백으로 흐름이 바뀐」 경우 `escapeOffered`가
    `true`로 남는다. 그러면 다음 흐름의 `showLoading`이 조기 반환해
    **탈출 버튼이 처음부터 보이고 20초 타이머도 걸리지 않는다.**
    흐름이 끝나는 지점이 아니라 **시작하는 지점**에서 지워야 모든 경로가 덮인다.
  - **★ 한 번 내준 탈출구는 도로 거두지 않는다** (v1.5.1에서 정정).
     `showLoading`은 한 흐름에서 **두 번** 불린다(«확인 중» → «불러오는 중»).
     무조건 `hidden = true`로 되돌리면 위에서 즉시 내준 버튼이 다음 단계에서 사라지고
     20초 타이머까지 다시 시작되어 **최악 40초**를 기다리게 된다. `escapeOffered`
     플래그로 «이미 내줬다»를 기록하고, 흐름이 끝날 때(`clearLoading`) 푼다.
   ...
   - **★ `failToLogin` 화면에는 «다른 계정으로 로그인»(로그아웃)이 함께 있어야 한다**
     (v1.5.1 신설). 이 화면은 구글 로그인 버튼을 지우는데, 실패 코드가 만료 계열이
     **아니면**(`unauthorized`·`forbidden`·`deactivated`·`server_error`)
     `app:session-expired`가 뜨지 않아 `renderLogin()`이 다시 불리지 않는다.
     그러면 «다시 시도 → 또 실패»만 반복할 수 있고 **로그아웃도 계정 전환도 못 한다** —
     공용 휴대폰에서 뒷사람이 들어올 방법이 아예 없어진다(§7).
