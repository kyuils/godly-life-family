/**
 * mccheyne-plan.js
 * Robert Murray M'Cheyne (1842) 성경읽기표 -- 클래식(Classic) 판.
 * 하루 4본문 = family(가정) 2 + secret(개인) 2. 신약.시편은 연 2회, 그 외 구약은 연 1회 통독.
 *
 * 데이터 출처(2개 이상 소스 대조 완료, 2026-07-18):
 *   1) willswire/mcheyne (GitHub) -- App/PlanConstants.swift RAW_PLAN_DATA (365일 전체 원본 배열)
 *      https://github.com/willswire/mcheyne/blob/main/App/PlanConstants.swift
 *   2) Ben Edgington, Interactive M'Cheyne Bible Calendar (edginet.org) -- classic 판. 1/1, 3/15,
 *      7/18, 8/8, 10/1, 12/31 대조 완료 -- 전부 일치(불일치 없음).
 *      http://www.edginet.org/mcheyne/calendar.php?cal=classic
 *
 * 영어 원문 참조(RAW_PLAN, "책명 장[:절-절][,장]" 형식)를 BOOK_MAP으로 한국어(개역개정 통용 명칭)로
 * 프로그램 변환한다(손 전사 아님). 변환 결과는 scripts/test-mccheyne.mjs 검증 대상이다.
 *
 * 브라우저: window.MccheynePlan.getReadings(dateStr)
 * Node:    require('./mccheyne-plan.js').getReadings(dateStr)
 */
(function (root, factory) {
  ...
  if (typeof root !== 'undefined') {
    root.MccheynePlan = api;
  }
})(typeof window !== 'undefined' ? window : (typeof globalThis !== 'undefined' ? globalThis : this), function () {
  'use strict';

  // 영어 성경 책명 -> 한국어(개역개정 통용 명칭) 매핑. 66권 전체.
  var BOOK_MAP = {
  "Genesis": "창세기",
  "Exodus": "출애굽기",
  "Leviticus": "레위기",
  ...
  // 길이 내림차순 정렬(접두어 매칭 시 "1 John"이 "John"보다 먼저 매칭되도록).
  var BOOK_NAMES_BY_LENGTH_DESC = Object.keys(BOOK_MAP).sort(function (a, b) {
    return b.length - a.length;
  });

  // day 1(1/1) ~ day 365(12/31) 원본(영어) 데이터.
  // 각 행 = [family 본문1, family 본문2, secret 본문1, secret 본문2]
  var RAW_PLAN = [
  ...
  if (RAW_PLAN.length !== 365) {
    throw new Error('MccheynePlan: RAW_PLAN must have exactly 365 days, got ' + RAW_PLAN.length);
  }

  // "Genesis 9-10" / "Psalms 78:1-37" / "Jeremiah 36,45" 같은 영어 참조를
  // "창세기 9-10장" / "시편 78:1-37" / "예레미야 36,45장" 형식의 한국어로 변환.
  function convertRef(ref) {
    var found = null;
    for (var i = 0; i < BOOK_NAMES_BY_LENGTH_DESC.length; i++) {
      var name = BOOK_NAMES_BY_LENGTH_DESC[i];
      if (ref === name || ref.indexOf(name + ' ') === 0) {
        found = name;
        break;
      }
    }
    if (!found) {
      throw new Error('MccheynePlan: unmapped book reference "' + ref + '"');
    }
    var rest = ref.slice(found.length).trim();
    var kor = BOOK_MAP[found];
    if (!rest) {
      throw new Error('MccheynePlan: missing chapter spec in "' + ref + '"');
    }
    // 절 범위(콜론 포함)는 "장" 접미사를 붙이지 않는다. 예: "시편 119:1-24"
    if (rest.indexOf(':') !== -1) {
      return kor + ' ' + rest;
    }
    return kor + ' ' + rest + '장';
  ...
  // 최초 로드 시 1회만 변환하여 캐시.
  var KOREAN_PLAN = RAW_PLAN.map(function (day) {
    return day.map(convertRef);
  });

  // 월별 누적 일수(평년 기준, 2월=28일). doy = CUM_DAYS[month-1] + day
  var CUM_DAYS = [0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334];
  var MONTH_MAX_DAYS = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];

  var DATE_RE = /^(\d{4})-(\d{2})-(\d{2})$/;

  /**
   * dateStr: 'YYYY-MM-DD' (연도는 무시하고 월/일로만 조회, 윤년 여부 무관)
   * 반환: { day: 1~365, family: [본문1, 본문2], secret: [본문1, 본문2] } | null
   * 2/29 요청은 2/28과 동일 본문을 반환한다.
   */
  function getReadings(dateStr) {
    ...
    if (month === 2 && day === 29) {
      day = 28; // 2/29 -> 2/28과 동일 본문 (윤년 여부 무관)
    } else {
      var maxDay = MONTH_MAX_DAYS[month - 1];
      if (day < 1 || day > maxDay) return null;
    }

    var doy = CUM_DAYS[month - 1] + day;
    if (doy < 1 || doy > 365) return null;

    var readings = KOREAN_PLAN[doy - 1];
    return {
      day: doy,
      family: [readings[0], readings[1]],
      secret: [readings[2], readings[3]]
    };
  }
